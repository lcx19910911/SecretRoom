    ...
    partial class EmailService : ServiceBase
    {
        ...
        private void SendEmail(object sender, System.Timers.ElapsedEventArgs e)
        {
            try
            {
                ...
                SendMail(Params.EmailAccount, title, sb.ToString(), @"C:\root\crm\OrderExecle\订单统计-" + DateTime.Now.ToString("yyyy-MM-dd") + @".xlsx,C:\root\crm\DrinkExecle\饮料统计-" + DateTime.Now.ToString("yyyy-MM-dd") + ".xlsx");
            }
            catch (Exception ex)
            {
